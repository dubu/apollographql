...
class UserAPI extends DataSource {
  ...
  async uploadProfileImage({
    ...
    const userId = this.context.user.id;
    if (!userId) return; // Create new S3 client instance
    ...
    const filename = uuidv4() + '.' + mime.getExtension(mimetype); // Upload the file to an S3 bucket using the createReadStream
    ...
    await s3.upload({
      ACL: 'public-read',
      // This will make the file publicly available
      Body: createReadStream(),
      Bucket: AWS_S3_BUCKET,
      Key: filename,
      ContentType: mimetype
    }).promise(); // Save the profile image URL in the DB and return the updated user
